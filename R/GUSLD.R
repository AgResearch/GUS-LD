...
GUSLD <- function(R6obj, SNPsets=NULL, SNPpairs=NULL, nClust=3, LDmeasure=c("r2"), output="matrix", file="LDresults"){
  ...
  ## check what output we are producing
  if(!is.vector(output) || length(output) != 1 || all(output == c("matrix", "file")))
    stop("Output argument is invalid. Needs to be either 'matrix' or 'file'")
  ## output a matrix
  if(output=="matrix"){
    if(!is.null(SNPpairs)) ## SNP pairs not available
      stop("Matrix output for given set of SNP pairs is not available. Try setting the 'output' argument to 'file'")
    else{
      ...
      if(is.null(SNPsets) || (is.list(SNPsets) && length(SNPsets) == 1 &&
        ...
        if(!is.null(SNPsets)){
          snps <- sort(SNPsets[[1]])
          if(any(snps) < 1 || any(snps) > R6obj$.__enclos_env__$private$nSnps || any(duplicated(snps)))
            stop("SNPset vector is invalid")
        }
        else
          snps <- 1:R6obj$.__enclos_env__$private$nSnps
        ...
        res <- foreach(snp1=iter(1:nSnps),.combine=GUSbase:::comb_mat,.multicombine=TRUE) %dopar% {
          ...
          for(snp2 in seq_len(snp1-1)){
            ...
            temp <- R6obj$.__enclos_env__$private$pfreq[ind]
            pA1_hat = temp[1]
            pA2_hat = temp[2]
      ...
      ## Block matric for estimation between two independent pair of SNPs
        ...
        if(any(snps1) < 1 || any(snps1) > R6obj$.__enclos_env__$private$nSnps || any(duplicated(snps1)))
          stop("SNPset vector is invalid")
        if(any(snps2) < 1 || any(snps2) > R6obj$.__enclos_env__$private$nSnps || any(duplicated(snps2)))
          stop("SNPset vector is invalid")
        ## Set up the clusters
        cl <- makeCluster(nClust)
        registerDoSNOW(cl)
        ...
        for(meas in 1:length(res)){
          rownames(res[[meas]]) <- R6obj$.__enclos_env__$private$SNP_Names[snps1]
          colnames(res[[meas]]) <- R6obj$.__enclos_env__$private$SNP_Names[snps2]
        }
        res[[length(res)+1]] <- list(Rows=R6obj$.__enclos_env__$private$pfreq[snps1],
                                      Columns=R6obj$.__enclos_env__$private$pfreq[snps2])
        res[[length(res)+1]] <- list(Rows=R6obj$.__enclos_env__$private$ep[snps1],
                                      Columns=R6obj$.__enclos_env__$private$ep[snps2])
        names(res) <- c(LDmeasure,"p","ep")
        return(res)
      }
      else
  ...
  ## write the results to a file
      ...
      if(is.null(SNPsets) || is.list(SNPsets) && length(SNPsets) == 1 &&
        ...
        res <- foreach(snp1=iter(1:nSnps),.combine=GUSbase:::comb_vec,.multicombine=TRUE) %dopar% {
          ...
          for(snp2 in seq_len(snp1-1)){
            ...
            LDvec[[meas+1]][snp2] <- c(R6obj$.__enclos_env__$private$SNP_Names[ind[1]], pA1_hat, R6obj$.__enclos_env__$private$ep[ind[1]],
                                       R6obj$.__enclos_env__$private$SNP_Names[ind[1]], pA2_hat, R6obj$.__enclos_env__$private$ep[ind[2]])
    ...
    ## Estimation for specified set of SNP pairs
      ...
      if(is.matrix(SNPpairs) && is.numeric(SNPpairs) && all(round(SNPpairs)==SNPpairs) && nrow(SNPpairs) == 2){
        pos_temp1 <- R6obj$.__enclos_env__$private$SNP_Names[SNPpairs[,1]]
        pos_temp2 <- R6obj$.__enclos_env__$private$SNP_Names[SNPpairs[,2]]
