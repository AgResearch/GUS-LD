...
RAdata <- readRA(rafile, snpsubset = subset)

deer <- makeUR(deer, filter=list(MAF=0.01, MISS=0.5, HW = c(-0.05,Inf), MAXDEPTH=500))
deer$writeRA(file="deer_filter.ra")
```


```{r est_LD}
nblocks = 3 # Number of blocks to break up the SNPs
nSnps = 38  # Number of SNPs in the dataset
## Where to cut the SNPs
...
deer <- makeUR(deer, filter = )

## Estimate LD
outfile <- filename=paste0("deer_r2_",iter,"_GUSLD.txt")
if(all(block1 == block2)){
  GUSLD(deer, nClust=nClust, filename=outfile, LDmeasure = "r2")
} else { 
  GUSLD(deer, SNPpairs = cbind(block1, block2), nClust=nClust, filename=outfile, LDmeasure = "r2")
}
```
